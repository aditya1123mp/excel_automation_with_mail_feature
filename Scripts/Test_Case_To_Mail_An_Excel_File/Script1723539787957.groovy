...
try {
    ...
    message.setSubject("Automation Test Results - Excel File")

    // Create a multipart message for the attachment
    Multipart multipart = new MimeMultipart()

    // Add attachment part (Excel file)
    MimeBodyPart attachmentPart = new MimeBodyPart()
    String filePath = 'C:\\Users\\DELL\\OneDrive\\Desktop\\mail\\add_employee_data.xlsx' // Replace with your Excel file path
    DataSource source = new FileDataSource(filePath)
    attachmentPart.setDataHandler(new DataHandler(source))
    attachmentPart.setFileName(new File(filePath).getName())
    multipart.addBodyPart(attachmentPart)

    // Set the complete message parts to the message
